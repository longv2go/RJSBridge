...
typedef BOOL (^RCTArgumentBlock)(RJSBridge *, NSUInteger, id);
...
#define RCT_NULLABLE_CASE(_value, _type) \
        ...
        default: {
          ...
          if (!strcmp(objcType, blockType)) {
            ...
            NSLog(@"Unsupported argument type '%@' in method %@.",
      ...
      NSAssert(i == numberOfArguments - 2,@"");
      RCT_ARG_BLOCK(
      ...
      NSAssert(i == numberOfArguments - 1, @"");
      RCT_ARG_BLOCK(
        if (RCT_DEBUG && ![json isKindOfClass:[NSNumber class]]) {
      ...
      NSLog(@"Unknown argument type '%@' in method %@. Extend RCTConvert"
    ...
    if (RCT_DEBUG) {
      ...
      if (nullability == RCTNonnullable) {
        ...
        argumentBlocks[i - 2] = ^(RJSBridge *bridge, NSUInteger index, id json) {
          if (json != nil) {
               ...
               arguments:(NSArray *)arguments
{
  ...
  if (RCT_DEBUG) {
    ...
    if (arguments.count != _argumentBlocks.count) {
      ...
      NSLog(@"%@.%@ was called with %zd arguments, but expects %zd",
  ...
  NSAssert(@encode(RCTArgumentBlock)[0] == _C_ID, @"");
