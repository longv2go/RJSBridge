...
+ (NSIndexSet *)NSIndexSet:(id)json
{
  ...
  for (NSNumber *number in json) {
    ...
    if (RCT_DEBUG && index < 0) {
      NSLog(@"Invalid index value %zd. Indices must be positive.", index);
    }
...
+ (RCTFileURL *)RCTFileURL:(id)json
{
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath:fileURL.path]) {
    NSLog(@"File '%@' could not be found.", fileURL);
    return nil;
  }
  return fileURL;
}
...
+ (NSDate *)NSDate:(id)json
{
  if ([json isKindOfClass:[NSNumber class]]) {
    ...
    if (!date) {
      NSLog(@"JSON String '%@' could not be interpreted as a date. "
...
NSNumber *RCTConvertEnumValue(const char *typeName, NSDictionary *mapping, NSNumber *defaultValue, id json)
{
  ...
  if (RCT_DEBUG && ![json isKindOfClass:[NSString class]]) {
    NSLog(@"Expected NSNumber or NSString for %s, received %@: %@",
                typeName, [json classForCoder], json);
  }
  id value = mapping[json];
  if (RCT_DEBUG && !value && [json description].length > 0) {
    NSLog(@"Invalid %s '%@'. should be one of: %@", typeName, json, [[mapping allKeys] sortedArrayUsingSelector: @selector(caseInsensitiveCompare:)]);
  }
  ...
  if ([json isKindOfClass:[NSArray class]]) {
    ...
    if (aliases.count) {
      ...
      for (NSString *alias in aliases) {
        ...
        if (number != nil) {
          NSLog(@"Using deprecated '%@' property for '%s'. Use '%@' instead.", alias, type, key);
   ...
   scaleMultiplier:(CGFloat)scaleMultiplier
{
  ...
  if ([UIFont fontNamesForFamilyName:familyName].count == 0) {
    ...
    if (font) {
      ...
      NSLog(@"Unrecognized font family '%@'", familyName);
      if ([UIFont respondsToSelector:@selector(systemFontOfSize:weight:)]) {
        font = [UIFont systemFontOfSize:fontSize weight:fontWeight];
